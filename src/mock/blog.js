...
Mock.mock(/^\/api\/blog(\?.*)?$/, function(options) {
  const i = options.url.indexOf("?") + 1;
  const datas = qs.parse(options.url.slice(i));
  return Mock.mock({
    code: 0,
    msg: "",
    data: {
